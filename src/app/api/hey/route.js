...
export async function GET(req) {
  ...
  try {
    ...
    let PriceData = null
    let PriceTagUrl = `https://www.goat.com/web-api/v1/product_variants/buy_bar_data?productTemplateId=${productId}&countryCode=MN`
    let recommendedProducts = []
    let recommendedUrl = `https://www.goat.com/web-api/v1/product_templates/recommended?productTemplateId=${productId}&count=8`

    // Fetch price data (with fallback)
    try {
      console.log('pricetag url' + PriceTagUrl)
    ...
    try {
      console.log('recommendedUrl ' + recommendedUrl)
